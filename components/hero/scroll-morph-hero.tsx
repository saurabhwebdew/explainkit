...
// NOTE: grid.x values (±330/±110) assume a wide desktop canvas. They are only
// ever used in the desktop scroll-scrub path below. Mobile uses a completely
// separate, plain CSS grid layout (see MobileCards) so it can never inherit
// these desktop-only pixel offsets and overflow a narrow viewport.
...
export function ScrollMorphHero() {
  ...
  return (
    <section
      ref={containerRef}
      id="home"
      className={simple ? "relative h-auto overflow-x-hidden" : "relative h-[220vh] md:h-[260vh]"}
    >
      <div className={cnSticky(simple)}>
        ...
        <div className="max-w-content mx-auto px-6 md:px-10 w-full relative">
          ...
          {simple ? (
            ...
            <div className="relative h-[340px] mt-20">
              {cards.map((card, i) => (
                <DesktopCard key={card.id} card={card} progress={progress} index={i} />
              ))}
            </div>
          )}
        </div>
...
// Mobile/reduced-motion layout: a genuine responsive CSS grid, not absolute
// positioning with desktop pixel offsets. This is the actual fix — the
// previous version reused grid.x values like ±330px meant for a wide desktop
// canvas, which on a ~375px phone screen placed cards fully outside the
// viewport (causing both horizontal overflow AND cards nobody could ever see
// "animate", since they were rendering off-screen the whole time).
function MobileCards({ prefersReduced }: { prefersReduced: boolean }) {
  return (
    <div className="grid grid-cols-2 gap-3 max-w-[360px] mx-auto mt-8 sm:mt-10">
      {cards.map((card, i) =>
        prefersReduced ? (
          <div key={card.id} className="aspect-[3/4] rounded-2xl shadow-lift">
            <CardVisual id={card.id} />
          </div>
        ) : (
          <motion.div
            key={card.id}
            className="aspect-[3/4] rounded-2xl shadow-lift"
...
// Desktop-only: scroll-scrubbed scatter → grid assembly. Untouched from the
// working desktop behavior — only reachable when !simple, i.e. a wide enough
// viewport for these pixel offsets to make sense.
function DesktopCard({
  ...
  return (
    <motion.div
      className="absolute w-[190px] h-[240px] -translate-x-1/2 shadow-lift"
